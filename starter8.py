...
def option_four(current_player, current_position):
    ...
    for scanning in range(len(the_map_dictionary)):
        if building_choice == the_map_dictionary[scanning]['Abbrev']:
            if (current_player['Current_Money'] - int(the_map_dictionary[modded_current_position]['BuildingCost'])) > 0:
...
def pass_go_checker(individual_player):
    if individual_player['Name'] == player_one['Name']:
        if (individual_player['Position'] % len(the_map_dictionary)) != 0:
...
def format_display(board):
    ...
    for y in range(len(board)):
        ...
        if position_string_one == board[y]['Position']:
            second_half += player_one['Symbol']
        if position_string_two == board[y]['Position']:
            second_half += player_two['Symbol']
        ...
        if board[y]['Building'] != 'No':
            second_half += '\u2302'
        completed_string = first_half.ljust(5) + middle.ljust(1) + second_half.ljust(5)
        string_for_function.append(completed_string)
...
if __name__ == '__main__':
    ...
    # appends new keys to the dictionary
